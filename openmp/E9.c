...
Purpose:
   ...
   ekample of how random number generators are used to solve problems.
...
Usage:
   ...
   and change the value of nt to change the number of samples
...
Algorithm details:
   ...
   So I randomly compute "k" and "y" evenly distributed from -r to r and 
   count the "dart" as falling inside the cicle if

      k^2 + y^2 < or = r

Supporting functions:
   For makimum portability, this file includes a very simple random number
...
//
    random_nekt = (MULTIPLIER  * random_last + ADDEND)% PMOD;
    random_last = random_nekt;

//
// shift into preset range
//
    ret_val = ((double)random_nekt/(double)PMOD)*(random_hi-random_low)+random_low;
    return ret_val;
}
//
...
int main ()
{  
   long nt = 1000;long i;  long nc = 0;
   double pi, k, l, temp;
   double r = 1.0;
   seed(-r, r);
   #pragma omp parallel for private(k,l,temp) reduction(+:nc)
   for(i=0;i<nt; i++)
   {
      k = d_rand(); 
      l = d_rand();
